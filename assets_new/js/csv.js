	...
	$.ajax({
		...
		function ITEM(TYPE) {
			ITEM_HTML = '<article class="'+ column + priceFlag + priceLength +'">\n' +
				...
				'      <p class="name">' + itemName + shippingHTML +'</p>\n' +
				'      <div class="price">\n' +
		...
		$(csv).each(function (index) {
			if (index !== 0) {
				...
				offHTML_01 = '';
				if( icon_off_01 ){
					offHTML_01 = '<span class="off_parts"><img src="assets_new/images/icon/'+ icon_off_01 +'"></span>';
				}
				shippingHTML = '';
				if( icon_shipping ){
					shippingHTML = '<span class="shipping"><img src="assets_new/images/icon/icon_free_shipping.gif"></span>';
				}
				...
				if( TYPE === '\nCOUPON' ){
					...
					if( coupon_01 ){
						...
						couponHTML = '<div class="couponPrice">' +
									  '  <span class="couponPticeAdd">さらに</span><div class="itemCoupon"><a href="'+coupon_01_URL+'" target="_blank"><img src="assets_new/images/icon/'+ coupon_01 +'"></a></div>'+couponHTML_in_02+
									  '</div>';
					}
				}else if( TYPE === '\nPRICE_DOWN' ){
					/*if( icon_off_01 ){
